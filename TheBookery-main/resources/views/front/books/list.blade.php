                    ...
                    @include('admin.message')
                        <div class="card-body table-responsive p-0">
                            <table class="table table-hover text-nowrap">
                                ...
                                <tbody>
                                    @if ($sellers1->isNotEmpty())
                                        @foreach ($sellers1 as $seller)
                                        @php
                                             $bookImage = getBookImage($seller->book_id)
                                        @endphp
                                        <tr>
                                        <td>{{ $seller->book->id }}</td>
                                        <td>
                                          @if (!empty($bookImage))  
                                        ...
                                        <td><a href="#">{{ $seller->book->title }}</a></td>
                                        <td>Ksh.{{ $seller->book->price}}</td>
                                        <td>{{ $seller->book->qty}} left in Stock</td>
                                        <!--<td>UGG-BB-PUR-0</td>-->
                                        <td>
                                            @if ($seller->book->status == 1)
                                        ...
                                        <td>
                                            <a href="{{ route('books.edit',$seller->book->id) }}">
                                            ...
                                            <a href="#" onclick="deleteBook({{ $seller->book->id }})" class="text-danger">
                                                <svg style="width: 16px; height: 16px;" class="filament-link-icon mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
